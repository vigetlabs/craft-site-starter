...
| [Imager X](https://plugins.craftcms.com/imager-x)                                                       | `spacecatninja/imager-x`                            | Image optimization and transforms. Provides useful Twig shortcuts for generating transforms and placeholders.                                                                      | $99.00       | $59.00        |
| [Imager X AWS Serverless Transformer](https://plugins.craftcms.com/imager-x-aws-serverless-transformer) | `spacecatninja/imager-x-aws-serverless-transformer` | On-demand image transforms via AWS Lambda + CloudFront. Configured via `config/imager-x-aws-serverless-transformer.php`.                                                           | Free         | Free          |
| [Imager X Power Pack](https://plugins.craftcms.com/imager-x-power-pack)                                 | `spacecatninja/imager-x-power-pack`                 | Extends Imager X with additional transform options and helpers.                                                                                                                    | Free         | Free          |
...
> This section is a template — customize it for each project after setup.

Deploys are triggered automatically via GitHub Actions webhook. Push to the `staging` or
`production` branch to deploy. `bin/deploy` is invoked by the webhook; do not run it manually.

See [docs/deploy-cache-clearing.md](docs/deploy-cache-clearing.md) for the full cache inventory
and rules for what is (and is not) cleared on deploy.
